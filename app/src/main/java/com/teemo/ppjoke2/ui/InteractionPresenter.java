...
import com.teemo.ppjoke2.model.Comment;
import com.teemo.ppjoke2.model.Feed;
import com.teemo.ppjoke2.model.TagList;
import com.teemo.ppjoke2.model.User;
import com.teemo.ppjoke2.utils.UserManager;
import com.teemo.ppjoke2.view.ShareDialog;
...
//                                   .postValue(feed);
    ...
    private static boolean isLogin(LifecycleOwner owner, Observer<User> observer) {
        if (UserManager.get().isLogin()) {
            ...
            LiveData<User> liveData = UserManager.get().login(AppGloble.getApplication());
            if (owner == null) {
    ...
    private static void showToast(String message) {
        ArchTaskExecutor.getMainThreadExecutor().execute(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(AppGloble.getApplication(), message, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private static Observer<User> loginObserver(Observer<User> observer, LiveData<User> liveData) {
        return new Observer<User>() {
            @Override
            public void onChanged(User user) {
                liveData.removeObserver(this);//移除原来的观察者
                if (user != null && observer != null) {
                    observer.onChanged(user);//用新的观察者去处理结果
                }
            }
        };
    }
...
//                                    .postValue(feed);
    ...
    private static void deleteFeedInternal(long itemId, MutableLiveData liveData) {
